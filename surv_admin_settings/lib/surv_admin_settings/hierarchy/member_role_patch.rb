  ...
  module Hierarchy
    module MemberRolePatch
      def self.included(base)
        base.class_eval do
          after_commit :sas_update_project_description
          after_commit :sas_ensure_parent_membership_for_manager, on: :create
          after_commit :sas_ensure_child_leadership_roles_for_parent_manager, on: :create
          after_commit :sas_sync_hierarchy_for_member_change
          after_destroy :sas_update_project_description

          private

          def sas_update_project_description
          ...
          def sas_sync_hierarchy_for_member_change
            return unless member && member.project
            SurvAdminSettings.ensure_hierarchy_roles_for_manager(member)
          end

          def sas_ensure_parent_membership_for_manager
            return unless member && member.project
            ...
            if parent_changed
              description_text = SurvAdminSettings.generate_project_members_description(parent_project)
              parent_project.update_column(:description, description_text)
            end
          end

          def sas_ensure_child_leadership_roles_for_parent_manager
            return unless member && member.project
            return unless role_id.to_i == SurvAdminSettings::MANAGER_ROLE_ID

            leadership_role = Role.find_by_id(SurvAdminSettings::CHILD_LEADERSHIP_ROLE_ID)
            ...
            member.project.descendants.each do |child_project|
              ...
              if changed
                description_text = SurvAdminSettings.generate_project_members_description(child_project)
